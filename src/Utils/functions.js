...
export default {
	loadEvent: async function(client, classLoad, folderName, commonJs = false) {
		const pathEvent = path.resolve('.', 'src', folderName);
		...
		for (
			const fileName of readdirSync(pathEvent)
				.filter(fileName_ =>
			...
			try {
				const event = commonJs ? require(`../${folderName}/${fileName}`) : (await import(`../${folderName}/${fileName}`)).default;
